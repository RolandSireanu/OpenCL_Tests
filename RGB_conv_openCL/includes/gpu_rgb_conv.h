...
class GpuRgbConv : public RgbConv
{
    ...
    void convert_to_rgb(std::string outputImage) override
    {
        ...
        kp = fopen("../kernels/kernel_rgb_conv.cl","r");
        if (!kp) {
        ...
        ret = clGetDeviceIDs( platform_id, HW, 1, 
        ...
        clGetDeviceInfo(device_id, CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(cu), &cu, NULL);
        std::cout << "Max compute units = " << cu << std::endl;
        clGetDeviceInfo(device_id, CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(wg), &wg, NULL);
        std::cout << "Max workgroup size = " << wg << std::endl;
        clGetDeviceInfo(device_id, CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS, sizeof(wi), &wi, NULL);
        std::cout << "Max work item dimensions = " << wi << std::endl;
        clGetDeviceInfo(device_id, CL_DEVICE_MAX_WORK_ITEM_SIZES, sizeof(wis), &wis, NULL);
        std::cout << "CL_DEVICE_MAX_WORK_ITEM_SIZES = " << wis << std::endl;
        ...
        checkResult(before, rgb_buffer.get());
        checkHeader(before, rgb_buffer.get());
